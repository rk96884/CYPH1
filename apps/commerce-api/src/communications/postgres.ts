...
export class PostgresCommunicationRepository implements CommunicationRepository {
  ...
  async claimNext() {
    ...
    try {
      ...
      const inserted=await client.query(`INSERT INTO communication_deliveries(source_event_id,order_id,customer_id,template_key,deduplication_key,status,attempt_count,processing_started_at)
  ...
  async markSent(id:string,provider:string,reference:string){await this.pool.query("UPDATE communication_deliveries SET status='sent',provider=$2,provider_reference=$3,sent_at=now(),updated_at=now(),last_error_code=NULL,processing_started_at=NULL WHERE id=$1",[id,provider,reference]);}
  async markFailed(id:string,errorCode:string){await this.pool.query("UPDATE communication_deliveries SET status='failed',last_error_code=$2,available_at=now()+interval '5 minutes',updated_at=now(),processing_started_at=NULL WHERE id=$1",[id,errorCode]);}
}
