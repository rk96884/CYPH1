...
| Database recovery verification | Guarded source/restore comparison checks migration history and aggregate row counts without reading personal-data fields | First isolated logical backup/restore rehearsal passed on 21 September 2026 with 10 matching migrations and 23 matching table counts; paid Render PITR and off-platform schedule remain outstanding |
...
- Guarded real worker process-interruption/replacement recovery passed locally on 22 September 2026: a real child process was terminated after its durable claim, immediate reclaim remained lease-blocked, and replacement recovery preserved the same event key and CYPH/1 fulfilment idempotency key. Repeat this process-interruption rehearsal against Render development; external-provider idempotency and provider-outage exercises remain open. The managed Render development database interruption/recovery rehearsal passed on 22 September 2026; guarded worker database lease/reclaim/retry-exhaustion evidence passed locally and on Render development on 21 September 2026.
- Load and soak tests using synthetic records only; no production personal data.
- Paid Render PITR rehearsal and off-platform backup-schedule verification. The first isolated logical backup/restore rehearsal passed on 21 September 2026.
