...
| Multi-carrier and collection-point delivery | ADR, logical data model, commercial-validation pack and privacy/security baseline | Preferred direction recorded: generic **Locker / Collection Point**, CYPH/1-owned carrier abstraction, Sendcloud as leading aggregator candidate and InPost/Evri as candidate underlying carriers. ADR 0003 and the logical schema separate propositions, selections, services, shipments and fulfilment. `SHIPPING-COMMERCIAL-VALIDATION.md` provides the evidence pack; `SHIPPING-PRIVACY-AND-SECURITY.md` defines minimized data flows, selector/label/webhook controls, retention and threats. No migration or integration is approved; provider responses, final package facts, contracts and accountable approval remain outstanding. |
